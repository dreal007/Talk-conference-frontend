...
export default new Vuex.Store({
  ...
  actions: {
    async get_talks({ commit }){
     ...
     async get_talk_by_id({ commit }, id){
      const [talk, err] =  await $http.get('talks/'+id);
      console.log (talk)
      if(talk && !err){
          commit('store_talk', talk.data.payload);
      }
    },
